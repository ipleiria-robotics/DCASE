...
def main():
    ...
    dcase_util.utils.setup_logging(
        logging_file=os.path.join(param.get_path('path.log'), 'task1a_v2_em.log')
    )
    ...
    for fold in folds:
        ...
        if not os.path.isfile(fold_model_filename) or overwrite:
            ...
            keras_model = tf.keras.Sequential([
                ...
                tf.keras.layers.Conv2D(filters=16, kernel_size=(7, 7), padding='same', name="Conv-1"),
                ...
                #
                # CNN layer #3:
                tf.keras.layers.Conv2D(filters=32, kernel_size=(7, 7), activation='relu', padding='same',
                                       name="Conv-3"),
                tf.keras.layers.BatchNormalization(),
                tf.keras.layers.Activation('relu'),
                tf.keras.layers.MaxPooling2D(pool_size=(4, 10)),
                tf.keras.layers.Dropout(0.3),

                tf.keras.layers.Flatten(),
                tf.keras.layers.Dense(100, activation='relu', name="lastLayer"),
                tf.keras.layers.Dropout(0.3),
            ...
            batch_generator = BatchGenerator('features_all_1.h5', batch_size=100)
            # Quantization to int8
...
def do_testing(db, scene_labels, folds, param, log, overwrite=False):
    ...
    for fold in folds:
        ...
        # interpreter = tf.lite.Interpreter(model_path=fold_model_filename,num_threads=12) #TF2.3
        ...
        if not os.path.isfile(fold_results_filename) or overwrite:
            ...
            features, _, filenames = get_data('features_all_1.h5', index_files)
            ...
            try:
                ...
                for i, feat in enumerate(features):
                    ...
                    # input_data = feat.reshape(1, feat.shape[0], feat.shape[1], 1)#.astype(input_details["dtype"])
                    #
                    # input_data = tf.convert_to_tensor(input_data, input_details["dtype"])
