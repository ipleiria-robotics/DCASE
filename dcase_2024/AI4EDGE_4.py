...
current_dir = current_dir + "/myDCASE"
...
for x in range(len(scene_labels)):
    ...
    try:
      globals()[f"keras_model_{x}"]=loadModelH5(directory+modelName)
        ...
        x1, y1 = divide2Emsemle(X_train, Y_train, label) #Fazer um emsemble de 10 modelos de modo a criar um teacher e por KD criar um modelo de menor dimensão
        # data = np.array([y1]).T
...
path_teacher = testNameDir + fold_model_filename   ################################################## ALTERAR#############
...
path_student = testNameDir + fold_model_filename  ################################################## ALTERAR #############
# if teacher model isnt quantized
...
train_student_with_response_kd=True
