...
def get_tflite_size(model):
    ...
    from NeSsi import tflite_tools as tflt
...
def get_model_size(model, model_type='keras', input_size=None):
    ...
    return macc, params
    # validate(macc, params,log)
    
def validate(macc, params,log):
    print('Model statistics:')
    log.line('Model statistics:')
    print('MACC:\t \t %.3f' %  (macc/1e6), 'M')
    log.line('MACC:' + str((macc / 1e6)) + 'M')
    print('Params:\t \t %.3f' %  (params/1e3), 'K\n')
    log.line('Params:' + str((params / 1e3)) + 'k')
    if macc>MAX_MACC:
        print('[Warning] Multiply accumulate count', macc, 'is more than the allowed maximum of', int(MAX_MACC))
        log.line('[Warning] Multiply accumulate count ' + str(macc) + ' is more than the allowed maximum of ' + str(
            int(MAX_MACC)))
    if params>MAX_PARAMS:
        print('[Warning] parameter count', params, 'is more than the allowed maximum of', int(MAX_PARAMS))
        log.line('[Warning] parameter count ' + str(params) + ' is more than the allowed maximum of ' + str(int(MAX_PARAMS)))